...
All constraints in this section hold **per instance** (see "Instances and instance sets" below). Additional constraints: `MIN_PIECE_LEN >= 1`; `MIN_PIECE_LEN <= MAX_PIECE_LEN`; `MIN_PIECE_LEN <= LAYER_LEN`; `NUM_LAYERS >= 1`; `OBSERVABLE_BOARDS >= 1`; `INI_PIECES >= INI_BOARDS`, so that every board holds at least one piece. A piece-count partition must also exist: some assignment of `INI_PIECES` across the `INI_BOARDS` boards gives every board `b` holding `n_b` pieces a non-empty length interval, `max(MIN_BOARD_LEN, n_b * MIN_PIECE_LEN) <= min(MAX_BOARD_LEN, n_b * MAX_PIECE_LEN)`. Without it a config can satisfy every constraint above and still admit no valid initial queue. If `MAX_PIECE_LEN > LAYER_LEN`, cuts producing pieces longer than `LAYER_LEN` can never be assembled and are effectively wasted. Additionally, the config must pass a base-case feasibility check (`reachable(0)`, defined in assemble constraint 4 below, using only the global forbidden intervals since layer 0 has no previous layer) to ensure the first layer of a beam can always be finished.
...
- **Instance set** — a training or evaluation set, drawn at two levels: it holds a set of instance-constant configs, each with a probability, the probabilities summing to 1; and for each config, a distribution over boards. An instance is drawn by choosing a config by its probability and then drawing boards under that config. One policy is trained across a set. A set holding a single config is the degenerate case of this, not a separate mechanism.

### Initial queue

An instance's starting material must satisfy the following. `INI_BOARDS` original boards hold `INI_PIECES` pieces in total. Every board is split into at least one piece, and each piece length is an integer in `[MIN_PIECE_LEN, MAX_PIECE_LEN]`. A board's length is the sum of its pieces' lengths and lies in `[MIN_BOARD_LEN, MAX_BOARD_LEN]`. Boards and pieces carry the `board_id` and `piece_id` listed in the hidden-variables table.

This fixes what a valid initial queue *is*, not how one is drawn: the distribution over board lengths and over the split of `INI_PIECES` across boards is a property of the instance set, chosen per config, and deliberately not fixed here.
