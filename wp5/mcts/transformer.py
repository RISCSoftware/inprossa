...
def demo():
    ...
    # These hyperparameters are taken from the "tiny" ViT models, which have ~5.7M parameters.
    # See vit_demo.py for details.
    ...
    dummy_x = jnp.ones((1, 16, 192))  # (batch, seq_len, hidden_size)
    params = model.init(jax.random.PRNGKey(0), dummy_x)

    n_params = sum(p.size for p in jax.tree.leaves(params))
    print(f"Parameter count: {n_params:,}")
    dtypes = set(p.dtype for p in jax.tree.leaves(params))
