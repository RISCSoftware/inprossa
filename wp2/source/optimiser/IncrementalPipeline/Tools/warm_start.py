...
def warm_start(new_model, previous_model):
    previous_vars = previous_model.getVars()

    for var in previous_vars:
        warm_start_vars_with_same_name(new_model, var)
...
def warm_start_vars_with_same_name(new_model, var):
    """
    ...
    same_name_vars = get_vars_by_name(new_model, var.VarName)
    if len(same_name_vars) > 1:
        # Give warning
        print(f"Warning: Multiple variables with the name {var.VarName} ",
              "found in the new model.")
    elif len(same_name_vars) == 0:
        # Give warning
        print(f"Warning: No variable with the name {var.VarName} ",
              "found in the new model.")
    else:
        # Set the start value of the variable in the new model
        for same_name_var in same_name_vars:
            same_name_var.start = var.X
