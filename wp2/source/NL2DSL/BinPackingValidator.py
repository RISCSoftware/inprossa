...
# This file contains methods for 2d bin packing validation.
...
def validate_solution(solver_solution : dict, task : dict, model = None):
    ...
    Args:
        ...
        model: ToT node or PolishModel, saves solver metadata (i.e., objective value).
    """
...
def _extract_assignment_and_position(solver_solution: dict):
    ...
    Special Note: this validator can handle the extraction of decision variables with different shapes
    Args:
    ...
    for i, item_positions in enumerate(solver_solution["x_y_positions"][0]):
        ...
        elif isinstance(item_positions, list) and len(item_positions) == 2:
            solution[i].update({"x": item_positions[0]})
            solution[i].update({"y": item_positions[1]})
        else:
            raise ValueError("Unreadable format of x_y_positions of solver solution for validation.")
    # Extract item-box-assignments
