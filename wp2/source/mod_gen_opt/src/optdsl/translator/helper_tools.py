...
class ExpressionRewriter:
    def __init__(self, loop_scope = dict(), variable_table=dict(), constant_table=dict(), types=dict(), code_block=None, dsl_index_base: int = 0):
        ...
        if code_block is not None:
            ...
            self.dsl_index_base = code_block.dsl_index_base
        else:
    ...
    def rewrite_expr(self, expr):
        ...
        if isinstance(expr, ast.BinOp):
            ...
            index_str = self._rewrite_index_for_minizinc(index)
            ...
            if func_name == "range":
                ...
                if len(args) == 1:
                    ...
                    return f"{self.dsl_index_base}..{second}"
                elif len(args) == 2:
    ...
    def _rewrite_index_for_minizinc(self, index_expr):
        """Convert DSL index into MiniZinc index according to configured base."""
        index_str = self.rewrite_expr(index_expr)
        if self.dsl_index_base == 1:
            return index_str
        if isinstance(index_str, str) and index_str.lstrip("-").isdigit():
            return str(int(index_str) + 1)
        if isinstance(index_expr, ast.Constant) and isinstance(index_expr.value, int):
            return str(index_expr.value + 1)
        return f"({index_str} + 1)"
