...
class CodeBlock:
    ...
    def __init__(self, *, constant_table=None, predicates=None, types=None, dsl_index_base: int = 0):
        # varname → Variable
        self.variable_table = {}
    ...
    def find_original_variable_and_assign(self, lhs, rhs_expr=None, rhs=None, loop_scope=None):
        ...
        while obj_name not in self.variable_table and obj_name not in self.constant_table:
            ...
            if isinstance(my_lhs, ast.Attribute):
                ...
                rewriter = ExpressionRewriter(loop_scope, code_block=self)
                subscript = rewriter._rewrite_index_for_minizinc(my_lhs.slice)
    ...
    def execute_block_for(self, stmt, loop_scope):
        ...
        # Case 3: iterate over constant name: for x in PIECES
            ...
            if const_name in self.constant_table:
                ...
                if self.dsl_index_base == 0:
                    iter_values = list(range(n))
                else:
                    iter_values = list(range(1, n + 1))
                loop_vars = [stmt.target.id]
        ...
        for item in iter_values:
            new_scope = self._bind_loop_variables(stmt, loop_scope, loop_vars, meta, item)
            self.execute_block(stmt.body, new_scope)

    def _resolve_range_iter(self, stmt, loop_scope):
        
        if len(stmt.iter.args) == 1:
            # range(end) starts from configured DSL index base.
            start_node = ast.Constant(value=self.dsl_index_base)
    ...
    def _resolve_enumerate_iter(self, stmt):
        ...
        if isinstance(arg, ast.List):
            ...
            iter_values = list(enumerate(values, start=self.dsl_index_base))
        ...
        # enumerate over constant array like PIECES
            ...
            const_obj = self.constant_table[arg.id]
            if isinstance(const_obj.type, DSList):
                n = const_obj.type.length
            else:
                raise ValueError(f"Unsupported constant type for enumerate: {const_obj.type}")
            if self.dsl_index_base == 0:
                iter_values = [(i, f"{arg.id}[{i + 1}]") for i in range(n)]
            else:
                iter_values = [(i, f"{arg.id}[{i}]") for i in range(1, n + 1)]
            loop_vars = [elt.id for elt in stmt.target.elts]
            # loop_vars: ['current_index', 'piece']
            ...
            versioned_const_name = var_obj.versioned_name()
            if self.dsl_index_base == 0:
                iter_values = [(i, f"{versioned_const_name}[{i + 1}]") for i in range(n)]
            else:
                iter_values = [(i, f"{versioned_const_name}[{i}]") for i in range(1, n + 1)]
            loop_vars = [elt.id for elt in stmt.target.elts]
    ...
    def _bind_loop_variables(self, stmt, loop_scope, loop_vars, meta, item):
        """
        ...
        if meta.get("kind") == "const":
            ...
            if self.dsl_index_base == 0:
                new_scope[loop_var] = f"{array_name}[{item + 1}]"
            else:
                new_scope[loop_var] = f"{array_name}[{item}]"
            return new_scope
