# Job Shop Scheduling Problem: 2 jobs x 2 machines
...
# Job 0: op0->machine0, op1->machine1
# Job 1: op0->machine1, op1->machine0
...
# completion[j][op] = completion time of operation op of job j
completion: DSList(N_JOBS, DSList(N_MACHINES, DSInt(0, 100)))

def precedence_constraint(completion: DSList(N_JOBS, DSList(N_MACHINES, DSInt(0, 100)))):
...
def no_overlap_constraint(completion: DSList(N_JOBS, DSList(N_MACHINES, DSInt(0, 100)))):
    # No two jobs can use the same machine at the same time
    # Check all pairs of operations on the same machine
...
def makespan(completion: DSList(N_JOBS, DSList(N_MACHINES, DSInt(0, 100)))):
    ms: DSInt() = 0
    for j in range(N_JOBS):
...
objective: DSInt(0, 100) = makespan(completion)
